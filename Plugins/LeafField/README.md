...
插件自带 `N_Example` Niagara System，可用于快速预览落叶飘落与人物交互的完整效果。

**使用方法：**

1. 在内容浏览器中找到 `Plugins/LeafField Content/LeafField/N_Example`
2. 直接将其拖入关卡，放置在**高度为 0 的地面上**（Z = 0）
3. 确保场景中已有地面网格（地板 Z = 0），落叶会从上方飘落到地面
4. 按照下方「快速上手」完成角色绑定后，进入 PIE 操控角色走过粒子区域，即可同时看到**落叶飘落到地面**以及**角色扰动叶片**的交互效果

> **注意：** 人物交互效果需要先完成角色绑定（第一步），否则只能看到落叶飘落，不会有角色交互。
